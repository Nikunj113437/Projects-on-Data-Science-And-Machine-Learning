...
api_key = "AIzaSyAQYxUoavidSpB_gbwxCJlf0zNkzOGPUC4"
...
def extract_videos(n, singer_name, dur, output_file):
    ...
    download_path = os.path.join(desktop, "Singer_X_Videos")
    ...
    def download_videos(video_url, count, n, download_path):
        try:
            ...
            video_title = yt.title
            print(
                f"Skipping video: {video_title} as it is unavailable for Download!")
            print(f"Video URL: {video_url}\n")
            return

        video_duration = yt.length
        if video_duration < int(dur) or video_duration > 480 or video_duration < 60:
            print(
    ...
    desktop = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
    audio_path = os.path.join(desktop, "Video_x_Audio")
    ...
    print("\nAudio File with .mp3 extension if Formed Successfully on Desktop!")
...
if __name__ == "__main__":
    try:
        ...
        print("Something Issue with Internet Connectivity, Please try Again!!")
